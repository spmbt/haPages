...
// @update 137 namespaces; похожие посты; сайдбар компаний;
		...
		if(!RegExp('^('+ HRU.replace(/\/\//,'//(\\w+\\.)*').replace(/ahabr/,'(ahabr)?\\') +'|/(?!\/)|#)','i').test(lH) && !/^textDecorationColor/.test(LI.className) ){
			...
			if((qaMatch = lH.match(/\/q\/(\d+)/)) && !/habracut/.test(LI.className)){ //вопросы-даты
				var postNum = qaMatch[1]
					,postYM =[0,0,0,0,0,0,0,0,1,983,1804,2711 //по 12 чисел с янв 2010
						...
						,61336,68300,75230,82958,91033,99151,107675,116875,126229,135799,147005,159003]; //дек.2014 qa -прогноз
			...
			var postNum = postMatch[2]
				...
				,postYM =[79815,82680,85884,89552,92535,95099,97893,100688,103185,105320,107313,109124
					...
					,207968,211020,214263,217769,221545,224805,228191,231847,235117,238753,242143,245701]; //дек.2014 -прогноз
...
var css='body{text-align: inherit!important; font-family: Verdana,sans-serif!important}'
	...
	+'.post div.btnBack, .btnBack{position: relative; height: 12px; margin: 3px 1px 5px 0; padding:0 3px!important; border: 1px solid #b3d2ee;border-radius:3px; font-size: 12px; background: #d3e2f0;color:#8ac;cursor:pointer}'
	...
	+'.showComm.btnBack.n2 .percent0{float:right} .showComm.btnBack.n2 .percent{vertical-align: top; border: 1px solid #b3d2ee; margin: -1px 3px 0; padding: 0 13px 1px; line-height:11px; background-color: #ecefee}'
	...
	+'.vacancies .job_item,'
	+'.tasks .task{margin-left: 1.6ex!important; margin-bottom: 0!important; line-height:  1.18!important; text-indent:-2ex}'
	+'.vacancies .job_item,.tasks .task{margin-left:0!important}'
