...
// @update 12 подгонка эллипсов к центрам чисел для комбинаций Habrajax, ZenComment или их отсутствия
		...
		if(oXS && oXS.getAttribute('title')){
			...
			if(oXSt && oXSt.length && !$q('canvas', o)){
				...
				if(oPPI){ //article
					...
					c.style.top = -7 + (hajax && 2) +'px';
				}else{ //comments
					oXS.style.left = (abs ? -1 : 7)+'px';
					oXS.style.top =0;
					c.style.left = -r2 + (abs >99 ? 18 : (abs >9 ? 11 : (abs ? 7 : 8))) -(isC2 && 1) +(!zen && 2) +'px';
					c.style.top = (isC2 ? -5 + zen : -3) -(zen && 4) +'px';
				}
